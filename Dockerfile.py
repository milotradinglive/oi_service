...
ARCHIVO_CRED   = os.getenv("GOOGLE_APPLICATION_CREDENTIALS", "credenciales_milo.json.json")
...
# ========= Auth flexible (ADC / JSON local) =========
def make_gspread_and_creds():
    try:
        adc_creds, _ = google_auth_default(scopes=SCOPES)
        print("🔐 Auth: Application Default Credentials (ADC)")
        return gspread.authorize(adc_creds), adc_creds
    except Exception as e:
        print(f"⚠️ ADC no disponible ({e}). Usando JSON local…")
        legacy_scopes = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
        legacy_creds = ServiceAccountCredentials.from_json_keyfile_name(ARCHIVO_CRED, legacy_scopes)
...
def procesar_autorizados(accesos_doc, main_file_url):
    ...
    # Directory API (requiere DWD). Si no tienes DWD en Cloud, puedes omitir membresía y solo marcar “ACTIVO”.
    try:
        ...
        creds_delegated = gsa.Credentials.from_service_account_file(ARCHIVO_CRED, scopes=SCOPES_DIR, subject=ADMIN_SUBJECT)
        ...
        for i, raw in enumerate(rows[1:], start=2):
            ...
            if not any([email, dur_txt, rol, creado, expira, estado, perm_id, nota]): continue
            if email.lower() == sa_email.lower():
